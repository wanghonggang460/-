...
def compareDateTimeStr(dataStr1, dataStr2):
    ...
    :return:
    """
    return datetime.datetime.strptime(dataStr1,
...
def get_minutes_between_two_unix_time(unix_begin, unix_end):
    ...
    """
    fmt = "%Y-%m-%d %H:%M"
    diff_sec = strToSec(secToStr(unix_end, fmt), fmt) - strToSec(
        secToStr(unix_begin, fmt), fmt)
...
def add_month(date, delta=1, cycle_id='month'):
    """
    求某一时间向前/向后,加/减多少小时/天/周/月/年之后的时间
    :param date: 时间
    :param delta: 数量
    :param cycle_id: 类型
    :return:
    ...
    if cycle_id == 'hour':
        return date + relativedelta(hours=delta)
    if cycle_id == 'week':
        return date + relativedelta(weeks=delta)
...
def get_next_day_time(date):
    """
    获取当天的下一天凌晨的时间（%Y-%m-%d 00:00:00）
    :param date: datetime
    :return:
    """
    next_day = date + datetime.timedelta(days=1)
    time_str = next_day.strftime('%Y-%m-%d 00:00:00')
    return time_str
...
def get_month_duration(start_time, end_time, bill_cycle='month'):
    logger.info('start_time: %s, end_time: %s, bill_cycle: %s', start_time,
                end_time, bill_cycle)
    n = 0
    if bill_cycle == 'day':
    ...
    logger.info('next_time: %s, start_time: %s, end_time: %s', next_time,
                start_time, end_time)
...
def create_time_7_24_1(add_time):
    ...
    before_month = get_now_month_day(date_now)
    # 比较一下,取小的
    start_time_24h = compareDateTime(add_time,
                                     before_day) and add_time or before_day
    start_time_7d = compareDateTime(add_time,
                                    before_day7) and add_time or before_day7
    start_time_1m = compareDateTime(add_time,
                                    before_month) and add_time or before_month
    datetime_dict = {
        'add_time': dateTimeToStr(add_time),
        'start_time': dateTimeToStr(start_time_24h),  # 默认24h
...
if __name__ == '__main__':
    ...
    # start_time = strToDateTime('2017-04-01 09:05:07')
    # end_time = strToDateTime('2017-05-01 00:00:00')
    #
    # duration = get_month_duration(start_time, end_time)
    # print(duration)
